...
elseif(empty($dure)){
	...
	$query5="SELECT * from reservation_salletp where reservation_date="."'".$date."'"." && reservation_heure="."'".$heure."'"." && nom_salletp="."'".$salletp."'"."    ";
//die(print_r($query5));
...
}else{$query3="INSERT INTO reservation_salletp (CNE,reservation_date,reservation_heure,nom_salletp,dure) values("."'".$CNE."'".","."'".$date."'".","."'".$heure."'".","."'".$salletp."'".",  "."'".$dure."'"." )           ";
	//die(print_r($query3));
...
if(isset($_POST['submit2'])){
	$date=htmlentities(trim($_POST['date']));
$query4= " SELECT nom_salletp,reservation_date,reservation_heure,dure from reservation_salletp where reservation_date >= "."'".$date."'"." && reservation_date <= ADDDATE("."'".$date."'".",7);  ";
//die(print_r($query4));
  ...
  <header>
	...
	<form class="terrain" action="" method="post">
		 ...
		 <input type="text" name="CNE" id="CNE" class="input" placeholder="<?php       echo    $CNE;       ?>"><br>
		                 <p> SALLE :</p>
		 <select class="input" style="color: black;" name="salletp" id="salletp">
          <?php    while($row=$result6->fetch_assoc()):  ?>
  <option class="dropdown-item" value="<?php echo $row['nom_salletp']  ; ?>"><?php echo $row['nom_salletp']  ; ?></option>
  <?php endwhile; ?>
</select>
